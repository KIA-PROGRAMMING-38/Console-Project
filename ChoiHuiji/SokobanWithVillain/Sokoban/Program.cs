...
class Program
{
    static void Main(string[] args)
    {
        ...
        Thread myThread = new Thread(SnailMove);
        myThread.Start();
        ...
        void SnailMove()
        {
            ...
            while (true)
            {
                ...
                if (direction == Direction.Right)
                {
                    while (snailX < MAX_X)
                    {
                        // 위에서 랜더하면 인풋을 넣어주기 전까지는 랜더하지 않으니까
                        // snail이 계속 움직이는게 보이기 위해서는 snail 함수에서 랜더해줘야 한다.
                        Render();

                        Thread.Sleep(100);
                        if (IsCollided(snailX + 1, snailY, player.X, player.Y)) continue;

                        for (int i = 0; i < walls.Length; ++i)
                        {
                            if (true == IsCollided(snailX + 1, snailY,
                                                walls[i].X, walls[i].Y))
                            {
                                collided = true;
                                break;
